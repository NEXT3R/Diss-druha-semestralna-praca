...
import VaccinationCentreSimulation.Events.VaccinationSystemEvent;
...
import VaccinationCentreSimulation.Events.PatientArrivalEvent;
...
public class VaccinationCentreSimulationCore extends EventSimulationCore {
    ...
    private double squaredWaitingRoomTime;
    ...
    private double releasedPatients;
    //stats
...
//        this.exportSamplesForTests();
    ...
    public void onWaitEnd(double waitTime) {
        releasedPatients++;
        this.waitingRoomTime += waitTime;
    }
    ...
    public double getReleasedPatients() {
        return releasedPatients;
    }

    public double getConfIntervalLeft() {
        if (actualReplication >= 30) {
            double s = Math.sqrt(getN() * ((squaredWaitingRoomTime + Math.pow(this.waitingRoomTime / this.actualSimulationTime, 2)) / this.actualReplication) -
                    Math.pow(getN() * ((simWRoomTime + waitingRoomTime / this.actualSimulationTime) / this.actualReplication), 2));
            return ((getSimWRoomTime() + ((getWaitingRoomTime() / getActualSimulationTime())))
                    / getActualReplication()) - ((s * 1.96) / Math.sqrt(actualReplication));
        } else {
            return -1;
        }
    }

    public double getConfIntervalRight() {
        if (actualReplication >= 30) {
            double s = Math.sqrt(getN() * ((squaredWaitingRoomTime + Math.pow(this.waitingRoomTime / this.actualSimulationTime, 2)) / this.actualReplication) -
                    Math.pow(getN() * ((simWRoomTime + waitingRoomTime / this.actualSimulationTime) / this.actualReplication), 2));
            return ((getSimWRoomTime() + ((getWaitingRoomTime() / getActualSimulationTime())))
    ...
    public double getN() {
        if (actualReplication >= 30) {
            return (1.0 / (actualReplication - 1));
        }
        return -1;
    }
...
//        System.out.println("Average vaccination queue length is: " + (this.simAvgVacQ / super.actualReplication));
    ...
    public void afterReplication() {
        if (actualReplication != replicationCount) {
            ...
            simWRoomTime += waitingRoomTime / this.actualSimulationTime;
            squaredWaitingRoomTime += Math.pow(this.waitingRoomTime / this.actualSimulationTime, 2);
            ...
            simAvgWaitQ += this.waitingRoomTime / this.releasedPatients;
